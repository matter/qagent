...
### 5.8 Portfolio、Risk、Rebalance、Execution、State
...
### 5.10 Agent Research、QA、Promotion
...
### 5.11 Production Signal、Paper、Reproducibility
...
paper 推进时会用策略信号中的 `planned_price`，缺失时用决策日 close。T+1 high/low 满足缓冲区间才成交，否则该计划单当日取消，并把失败原因写入 daily/trade diagnostics。
...
`create_execution_policy_spec_3_0` 可创建 `policy_type="planned_price"`。`construct_portfolio_3_0`、`simulate_strategy_graph_day_3_0` 和 `backtest_strategy_graph_3_0` 会沿用该执行策略；alpha row 可传 `planned_price`。
...
legacy MCP 的 `run_backtest.config_json` 和 `create_paper_session.config` 均可传 `execution_model="planned_price"`、`planned_price_buffer_bps=50`。缺省仍为 `next_open`，保证旧 agent 脚本兼容。
