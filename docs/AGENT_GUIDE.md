...
宏观数据是跨市场辅助数据源，不属于 `US/CN` 股票 market scope。当前只接入 FRED，配置在 `external_data.fred`，也可用环境变量 `FRED_API_KEY` 覆盖。不要把 FRED 数据默认当成严格历史发布回放；FRED API 默认返回当前实时窗口，`realtime_start/realtime_end/available_at` 只是当前返回版本的可见性字段。需要严格 PIT 宏观研究时，必须先实现按历史 realtime window 回放和质量标注。
