...
如果 graph 绑定 `planned_price` execution policy，`alpha_frame` 可以带 `planned_price`。回测执行会读取 T+1 high/low 和 T 日 close，诊断写入 `summary.fill_diagnostics`、`backtest_daily.diagnostics.execution` 和 `backtest_trades.metadata`。agent 验收时至少检查：
...
自定义 strategy 应 subclass `StrategyBase`，返回按 ticker indexed 的 DataFrame，包含 `signal`, `weight`, `strength`。V3.1 起 strategy 还可以声明策略默认参数，并在每个订单行输出成交意图。配置合并顺序固定为：
...
class MyStrategy(StrategyBase):
    ...
    default_paper_config = default_backtest_config
```

允许放入 strategy defaults 的是策略行为参数：`position_sizing`、`max_positions`、`max_position_pct`、`normalize_target_weights`、`rebalance_freq`、调仓缓冲、持仓/冷却天数、`execution_model`、计划价参数和 `constraint_config`。`market`、股票池、起止日期、资金、成本、benchmark、debug/task 持久化等运行环境字段禁止由 strategy defaults 设置，必须由 run/session 拥有。回测结果 `config` 会保存 `strategy_default_config`、`effective_config` 和 `config_provenance`，验收时优先看这三个字段确认参数来源。

V3.1 推荐 agent 把“策略天然属性”写进 strategy class，把“本次运行环境”留给 backtest 或 paper session：

- 写进 `default_backtest_config/default_paper_config`：仓位 sizing、最大持仓数、调仓频率、是否标准化目标权重、执行模式、计划价 buffer、计划价 fallback、策略内置风控约束。
- 运行时再传：`start_date/end_date`、`initial_capital`、交易成本、benchmark、`universe_group_id`、debug/replay、任务持久化和本次明确要实验覆盖的字段。
- 策略可以在不同股票或不同日期动态输出 `execution_model`。例如核心买入用 `planned_price`，紧急退出用 `next_close`，普通轮动用 `next_open`。逐订单字段优先级最高，会覆盖本次全局 config。
...
class PlannedMomentumStrategy(StrategyBase):
    name = "planned_momentum_v1"
    ...
    default_backtest_config = {
        ...
        "planned_price_buffer_bps": 50,
        "planned_price_fallback": "next_close",
        "constraint_config": {"max_single_name_weight": 0.10},
    }
    default_paper_config = default_backtest_config
    ...
    def generate_signals(self, context: StrategyContext) -> pd.DataFrame:
        ...
        latest = available[-1]
        score = close.loc[latest] / close.loc[available[-20]] - 1.0
        selected = score.dropna().nlargest(5)
        if selected.empty:
            return pd.DataFrame(columns=["signal", "weight", "strength"])

        rows = []
        for ticker, strength in selected.items():
            rows.append(
                {
                    "ticker": ticker,
                    "signal": 1,
                    ...
                    "execution_model": "planned_price",
                    "planned_price": float(close.loc[latest, ticker]) * 1.002,
                    "planned_price_buffer_bps": 50,
                    "planned_price_fallback": "next_close",
...
- `next_open`：T 日决策，T+1 open 成交。
- `planned_price`：T 日决策，T+1 high/low 判断计划价是否可成交。
- `next_close`：T 日决策，T+1 close 成交；不是同日收盘成交。

策略输出 DataFrame 可以额外提供逐订单成交意图列：
...
return pd.DataFrame(
    {
        ...
        "planned_price_fallback": ["next_close"],
        "time_in_force": ["day"],
        "order_reason": ["breakout planned entry"],
    },
    index=["AAPL"],
)
```

同一调仓日可以混用 `next_open`、`planned_price` 和 `next_close`。引擎会在 `trade_diagnostics.order_intents` 中记录每笔订单的 `decision_date`、`execution_date`、`ticker`、`target_weight`、`execution_model`、`fill_type`、`fill_status`、`fill_price`、`blocked_reason` 和 `order_reason`，并汇总 `execution_model_counts`。

没有 `planned_price` 或值无效时，系统使用决策日 close。策略返回空 DataFrame 表示目标清仓时，legacy backtest 也会给当前持仓写入决策日 close 作为退出计划价，避免清仓单因为缺计划价被误记为 `invalid_planned_price`。回测结果会在 `trade_diagnostics.planned_price_execution` 记录成交、失败和成交率，在 `trade_diagnostics.planned_price_inputs` 记录 fallback 计数和样本。失败订单不生成真实成交，不改变持仓和现金。
...
研究迭代中不要拉完整 backtest JSON 做判断。优先调用 `/api/strategies/backtests/research-summary`，它返回 baseline/trial 核心指标、delta、有限条 rebalance digest、交易数量、`decision.conclusion` 和 `reproducibility_diagnostics`。agent 对比不同 backend commit 的结果时，必须先看 `strictly_comparable`、`compatibility_flag`、`difference_sources`、`field_diffs` 和 `result_shape_delta`：如果是 `backend_change_same_inputs`，只能说明输入指纹一致但系统实现变了，不要把结果差异直接解释为策略变好或变差。大 payload 仍通过 `/rebalance-diagnostics` 分页或 debug replay 定点查看。
...
创建 paper session 时同样会读取 strategy `default_paper_config`。UI 默认继承策略默认参数，只有切换为覆盖时才提交仓位、调仓和成交模式字段。agent 通过 REST/MCP 创建 session 时也应优先只传运行环境字段，除非本次实验明确要覆盖策略默认值。
...
MCP 推荐同 REST 一样优先继承 strategy defaults。`run_backtest.config_json` 示例：
...
只有本次实验明确要覆盖策略行为时，才在 `config_json` 增加策略拥有字段：
...
```json
{
  ...
  "benchmark": "SPY",
  "execution_model": "planned_price",
  "planned_price_buffer_bps": 75,
  "planned_price_fallback": "next_close",
  "max_positions": 15
}
```

`create_paper_session.config` 示例：
...
如果要做 paper 层覆盖，再显式加入 `execution_model`、`planned_price_buffer_bps`、`planned_price_fallback` 或 `max_positions`。MCP 返回的 backtest 是 task，需要继续用 `get_task_status` 轮询；paper session 创建是同步返回 session。
...
创建带策略默认参数和动态成交意图的 legacy strategy：
...
payload = {
    "market": "US",
    "name": "planned_momentum_v1",
    "description": "Strategy defaults own execution and sizing; runs own date/capital/universe.",
    "source_code": source,
    "position_sizing": "raw_weight",
}
print(json.dumps(requests.post("http://127.0.0.1:8000/api/strategies", json=payload).json(), indent=2))
PY
```

继承策略默认参数运行 legacy backtest。这里不要重复传 `execution_model/max_positions/rebalance_freq`，除非本次实验就是覆盖这些策略行为：

```bash
curl -fsS -X POST http://127.0.0.1:8000/api/strategies/<strategy_id>/backtest \
  -H "Content-Type: application/json" \
  -d '{
    "market": "US",
...
创建继承策略默认参数的 legacy paper session：

```bash
curl -fsS -X POST http://127.0.0.1:8000/api/paper-trading/sessions \
  -H "Content-Type: application/json" \
  -d '{
    "market": "US",
...
验收 V3.1 backtest 配置继承和计划价成交：

```bash
curl -fsS http://127.0.0.1:8000/api/strategies/backtests/<backtest_id> | python -c '
import json
import sys

bt = json.load(sys.stdin)
cfg = bt.get("config", {})
diag = bt.get("trade_diagnostics", {})
print("strategy_default_config:", cfg.get("strategy_default_config"))
print("effective_config:", cfg.get("effective_config"))
print("config_provenance:", cfg.get("config_provenance"))
print("execution_model_counts:", diag.get("order_intents", {}).get("execution_model_counts"))
print("planned_price_execution:", diag.get("planned_price_execution"))
'
```

agent 验收规则：

- `config.strategy_default_config` 应显示策略源码声明的默认参数。
- `config.effective_config` 应等于系统默认、策略默认和本次 run/session override 合并后的最终配置。
- `config.config_provenance` 应能说明每个关键字段来自 `system_default`、`strategy_default` 还是 `run_override/session_override`。
- `trade_diagnostics.order_intents.execution_model_counts` 应符合策略输出意图；若同日混用成交模式，这里应能看到多个模式。
- `trade_diagnostics.planned_price_execution.fallback_close_rate` 很高时，只能说明大量计划单靠收盘价兜底，不应解读为计划价本身高质量成交。
